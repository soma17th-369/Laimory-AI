...
- 경로를 지정하지 않으면 `.env`, `.env.local`, `.env.dev`, `runtime-env.json`, `.venv` 중 현재 worktree에는 없고 주 워크트리에는 있는 항목만 후보로 삼는다.
...
스크립트는 `git rev-parse --path-format=absolute --git-common-dir`로 주 워크트리를 찾고 모든 대상을 먼저 검증한 다음 연결한다.
