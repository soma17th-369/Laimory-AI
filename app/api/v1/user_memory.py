...
App Server 는 202 만 접수 성공으로 보고 수 초 timeout 을 건다. 재시도 배치 계약인
``dailyTimelines`` 최대 5건은 요청 schema 가 강제한다. 그 안에서 event 가 많거나 본문이
길어도 거절하지 않고 프롬프트 조립 단계에서 자른다
(:mod:`app.services.user_memory_limits`).
