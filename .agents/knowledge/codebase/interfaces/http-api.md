...
- `app/server.py`, `app/api/v1/router.py`, `app/api/v1/timeline.py`, `app/api/agentcore.py`
- `app/api/error_handlers.py`, `app/api/request_logging.py`
- `app/schemas/error.py`, `app/schemas/task.py`
- `tests/api/**`
...
| `POST /invocations` | AgentCore 고정 호출 경로 | `/v1/timeline`과 같은 request/response와 handler 사용 |
...
- 202는 완료가 아니라 접수다. 최종 상태는 App Server callback으로만 통보한다.
