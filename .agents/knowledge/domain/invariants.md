...
- 알림에서 얻는 정보는 대화·결제·예약 셋이다(#116). 앱 정책(`app_dictionary.json`)은 Notion 「코드가 다루는 앱 목록」을 옮긴 것이고, 그 앱에서 **얻을 수 있는 정보만** 말한다. candidate 생성 여부·confidence 같은 사용 판단은 정책에도 코드에도 두지 않는다 — 키워드·정규식으로 알림을 먼저 분류하던 `timelineUseGuidance`·`messengerInterpretation`은 없앴다.
- 정책 매칭은 입력 `appName` 하나만 본다(공백·대소문자 무시, 별칭, 패키지명). `title`·`text`를 뒤지면 대화 본문의 `결제` 한 단어로 그 알림이 결제 앱이 된다.
- 정책이 없는 앱의 알림은 사용자가 직접 눌러 담은 알림으로 보고 같은 앱·같은 `title` 단위로 묶어 `conversations`로 넘긴다. 코드는 묶고 건수·시각·최대 간격만 계산하며 입력 알림을 버리지 않는다. 대화 candidate를 하루 최대 3개로 제한하는 판단은 prompt가 한다.
- 알림 입력에는 대화 참여자 목록이 없다. 카카오톡 단체방의 `title`은 방 이름이 아니라 보낸 사람일 수 있어, 같은 방이 보낸 사람별 묶음으로 나뉘어 올 수 있다.
