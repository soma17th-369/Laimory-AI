...
판단(실행계획 수립, 사용자 승인 후 작업 진행)은 SKILL.md를 따르는
Codex 또는 Claude가 담당하고, 이 스크립트는 이슈 데이터 조회만 수행한다.
"""
...
# .agents/skills/issue-plan/scripts/issue_plan.py 기준 저장소 루트
...
def build_parser():
    parser = argparse.ArgumentParser(description="issue-plan 스킬 헬퍼")
