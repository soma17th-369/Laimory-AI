...
_DESCRIBE_RESPONSE = json.dumps(
    {"descriptions": [{"id": 1, "description": "저녁 무렵 야외에서 남긴 사진으로 보인다."}]},
    ensure_ascii=False,
)
...
def test_describer_fills_only_missing_descriptions():
    photos = [
        PhotoItem(id=1, takenAt="2026-06-20T18:00:00", dateTaken=None),
        PhotoItem(id=2, takenAt="2026-06-20T19:00:00", description="이미 있는 설명"),
    ]
    llm = FakeLLM([_DESCRIBE_RESPONSE])

    out = LLMPhotoDescriber(llm).describe(photos)

    assert out == {1: "저녁 무렵 야외에서 남긴 사진으로 보인다."}
    assert len(llm.calls) == 1  # 대상은 사진 1장뿐이라도 배치 1회 호출


def test_describer_no_call_when_all_present():
    photos = [PhotoItem(id=1, takenAt="2026-06-20T18:00:00", description="있음")]
    llm = FakeLLM([_DESCRIBE_RESPONSE])
...
def test_agent_describes_then_infers_with_filled_description():
    request = make_request(
        photos=[PhotoItem(id=1, takenAt="2026-06-20T18:00:00", dateTaken=None)]
    )
    ...
    assert result.candidates and result.candidates[0].source_refs[0].source_id == "1"


def test_agent_skips_describe_when_description_present():
    request = make_request(
        photos=[PhotoItem(id=1, takenAt="2026-06-20T18:00:00", description="식탁 사진")]
    )
