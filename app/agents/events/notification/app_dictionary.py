...
(대부분 메신저). 사전에 없는 앱의 알림은 정책 없이 따로 실어 내용으로 판단하게 한다.

메신저는 표 밖에서 정책 둘을 둔다(#116). 개인 메신저(카카오톡·Instagram·Discord)는 사적인
대화에 더해 가게·서비스가 알림톡으로 보내는 예약·결제·배송 안내가 오고, 업무 메신저
(Webex·Slack·Microsoft Teams)는 업무·교육 대화와 회의 안내가 온다. 둘 다 알림은 대화 묶음으로
가고, 묶음이 자기 정책을 가리킨다.
