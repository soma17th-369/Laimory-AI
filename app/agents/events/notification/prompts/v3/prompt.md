...
- `conversations`: 사용자가 직접 눌러 담은 메신저 등의 알림을 같은 앱·같은 대화 상대(`title`) 단위로 묶은 것입니다. 정책이 없는 앱의 알림과 메신저(카카오톡·Instagram·Webex·Slack·Discord) 알림이 여기로 옵니다. 메시지 수가 많은 순서로 옵니다.
