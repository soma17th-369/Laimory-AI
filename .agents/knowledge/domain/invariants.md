...
- 정책이 없거나 대화를 주는 앱(메신저)의 알림은 사용자가 직접 눌러 담은 알림으로 보고 같은 앱·같은 대화 상대(`title`) 단위로 묶어 `conversations`로 넘긴다. 코드는 묶고 건수·시각·최대 간격만 계산하며 입력 알림을 버리지 않는다. 묶음의 메시지도 내용에 따라 결제·예약 근거가 된다. 대화 candidate를 하루 최대 3개로 제한하는 것은 지금 prompt만 지키며, Repair 단계 검사는 #119에서 한다.
