...
def _event(eid: str, raw_ids: list[str], **kw) -> TimelineEventDraft:
    defaults = dict(
        ...
        source_refs=[SourceRef(source_type="PHOTO", source_id=r) for r in raw_ids],
    )
    defaults.update(kw)
...
def test_valid_draft_has_no_violations():
    ...
    assert validate_timeline_for_storage(draft, {"raw-a", "raw-b", "raw-c"}) == []


def test_source_not_in_task_is_flagged():
    draft = _draft([_event("e1", ["raw-x"])])
    violations = validate_timeline_for_storage(draft, {"raw-a"})
    assert any("raw-x" in v and "task" in v for v in violations)


def test_source_reused_across_events_is_allowed():
    draft = _draft([_event("e1", ["raw-a"]), _event("e2", ["raw-a"])])
    assert validate_timeline_for_storage(draft, {"raw-a"}) == []
...
def test_ensure_valid_raises_on_violation():
    draft = _draft([_event("e1", ["raw-x"])])
    try:
        ensure_timeline_valid_for_storage(draft, {"raw-a"})
