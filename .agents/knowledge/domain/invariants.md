...
- 접수 입력은 크기로 거절하지 않고 prompt 조립 단계에서 자르며, 자를 때 `memo` 있는 event를 끝까지 남긴다. 무엇을 얼마나 잘랐는지는 운영 이벤트에 남긴다.
...
- User Memory 갱신 입력 상한(일기 7일·event 50개·`memo` 500자)도 이 저장소가 고른 방어선이다. App Server 쪽 실제 상한과 일치하는지는 여기서 확인할 수 없다.
