...
- 계획은 두괄식으로 쓴다. 첫 요약만 읽어도 「이슈를 연 이유인 현재 문제 → 해결 방향 → 완료 후 결과」가 바로 보여야 하고, 실행 단계·결정·검증이 그 뒤를 잇는다.
...
- 이슈 본문을 단순 축약하지 않는다. 관련 코드를 확인해 현재 문제가 실제로 어디에서 발생하는지,
  기존 계약을 보존하면서 어떤 방향으로 해결할지를 계획 요약에 연결한다. 확인되지 않은 원인은 추측하지 않는다.
