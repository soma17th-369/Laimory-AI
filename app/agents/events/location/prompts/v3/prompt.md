...
- 여러 MOVEMENT와 짧은 중간 STAY가 같은 방향의 연속된 이동을 가리키면 출발지와 최종 도착지를 중심으로 하나의 상위 여정 candidate를 생성합니다.
...
- 센서 누락으로 기록이 비었을 뿐이므로, 근거의 강도에 맞는 confidence와 표현을 사용합니다. 공백이 길수록 장소·활동의 확신을 낮춥니다.
...
- 제목은 `지역 간 장거리 이동`, `최종 목적지까지의 이동`, `오전 주거지 체류`, `저녁 근거리 외출`, `저녁 산책`처럼 실제 이동·체류 의미가 드러나게 작성합니다.
- 설명은 출발지, 최종 도착지, 체류 시간, 불확실성을 사용자가 읽을 수 있는 자연스러운 문장으로 작성합니다.
- 설명에는 병합한 이동·체류의 핵심 시간, 장소, 거리와 관측 공백을 자연스럽게 반영합니다.
