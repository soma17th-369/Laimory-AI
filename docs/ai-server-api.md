...
- `photoUrl` 값은 운영 로그와 LLM 프롬프트에는 남기지 않습니다. Langfuse trace의 요청
  덤프(`request.photos[].photoUrl`)에는 원문이 남아 어느 사진을 보고 만든 설명인지 확인할
  수 있습니다(이슈 #127). 콘텐츠 정책이 `NONE`인 환경에서는 본문 전체가 해시로 접힙니다.
