...
└── services/
    ...
    │                          #   (1) resolve_candidate_places (#72): candidate 의 place/
    │                          #       places/address 를 sourceRefs 로 찾은 입력에서 **그대로
    │                          #       복사**한다. Event Agent 는 채우지 않는다 — 한 지점에
    │                          #       이름이 여럿일 때 어느 것이 맞는지 판단할 근거가 없다.
    │                          #       places 를 줄이지 않는 이유도 그것이다(고르는 건 Timeline)
    │                          #   (2) placeLabel 을 근거 place 로 확정, 근거 없는 address 제거
    │                          #   (3) 보존 검사 (#72): Timeline 이 쓴 placeLabel 이 입력이나
...
#   → run_question_agent
#   merge_results 는 취합만 하지 않는다. `merge_event_results` 가 candidate 의 place/places/
#   address 를 입력에서 복사한다(#72). 이 자리인 이유는 Timeline Agent 로 들어가는 fan-in 이
