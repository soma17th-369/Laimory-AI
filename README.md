...
# 관측(Observability) — 마스킹·크기 제한한 Timeline 실행 JSON을 ES로 보낸다(#28).
...
OBS_CONTENT_CAPTURE=SANITIZED # 마스킹 본문 저장. NONE이면 길이·해시만
OBS_MAX_PAYLOAD_BYTES=16384  # 이벤트별 payload 최대 byte
OBS_MAX_EVENTS_PER_TASK=1000 # task별 메모리 버퍼 이벤트 상한
OBS_LOCAL_DIR=               # dev 검사용 task.json/events.jsonl 저장 경로
...
Timeline 요청(`taskId`) 하나의 입력·Agent·LLM·저장·콜백을 구조화 로그로 모아
**Elasticsearch → Kibana** 에서 `taskId` 로 조회한다. 입력·프롬프트·응답·오류는 민감정보
마스킹과 크기 제한을 적용해 저장한다. 운영 로그(FastAPI 등)는
...
- ES 인덱스 매핑: [task](docs/observability/agent-tasks-index-template.json), [event](docs/observability/agent-events-index-template.json)
