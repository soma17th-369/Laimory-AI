...
> 이 문서는 **AWS 웹 콘솔** 기준이다. AgentCore 배포의 상세 콘솔 절차는
> [AgentCore 배포 가이드](deploy-agentcore.md)를 참고한다.
...
**Amazon Bedrock → AgentCore → 해당 Agent Runtime → 환경 변수** 를 편집한다. 최초 생성은
[AgentCore 배포 가이드](deploy-agentcore.md) 6장, 기존 Runtime 변경은 9.2장의 새 버전 생성
절차를 따른다. 환경 변수를 바꾸면 새 Runtime 버전이 만들어진다.
...
- [AgentCore Runtime 배포 가이드](deploy-agentcore.md) — 6장 운영 환경 변수
