...
| 대화 묶음 (`conversations`) | 대화를 주는 앱(메신저)의 알림을 같은 앱·같은 대화 상대(`title`) 단위로 묶은 Notification Agent 입력(#116). 단체방 이름이 입력에 없어 방 단위가 아니다. 대부분 대화지만 예약·결제 안내일 수 있고, 판단은 prompt가 한다. Event Agent 출력 `candidate`와 다른 말이다. |
| 미분류 알림 (`unclassified`) | 사전에 없는 앱의 알림을 정책도 묶음도 없이 시각순으로 실은 Notification Agent 입력(#116). 무엇인지는 prompt가 내용만 보고 정한다. |
