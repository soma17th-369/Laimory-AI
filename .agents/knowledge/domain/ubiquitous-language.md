...
| 알림 정책 (`policyId`) | 한 도메인(금융·결제, 쇼핑·택배, 예약·문화, 이동·여행, 배달·카페, 문자)의 앱에서 얻을 수 있는 정보(#116). `provides`는 `PAYMENT`·`RESERVATION` 중 무엇을 주는지다. 정책이 붙는 앱은 사용자가 알림을 누르지 않아도 수집되는 앱이다. 사용 판단(candidate 여부·confidence)을 담지 않는다. |
| 대화 묶음 (`conversations`) | 정책이 없는 앱(사용자가 직접 눌러 담은 알림)의 알림을 같은 앱·같은 `title` 단위로 묶은 Notification Agent 입력(#116). 대부분 메신저 대화지만 대화가 아닐 수 있고, 판단은 prompt가 한다. Event Agent 출력 `candidate`와 다른 말이다. |
